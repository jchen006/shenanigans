...
def create_and_train_vae(network_architecture, recipe_vects, n_samples, learning_rate=0.0001,
          batch_size=50, training_epochs=10000, display_step=5, default_save_path='save/vae.ckpt'):
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                if (n_samples - prev_start_idx < batch_size):
                    prev_start_idx = 0
                recipe_batch = recipe_vects[prev_start_idx:prev_start_idx + batch_size]
...
if __name__ == '__main__':
    ...
    vae = create_and_train_vae(DEFAULT_NETWORK_ARCHITECTURE, X, n_samples, training_epochs=15)#216)
