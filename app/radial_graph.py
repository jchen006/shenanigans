...
class RadialGraph:
    ...
    def insert_ingred_obj(self, ingred_name, recipe_name):
        ...
        ingred_obj.recipes.add(recipe_name)
        return
    ...
    def make_d3_graph_json(self, root, K_=3, level_of_tree=3):
        ...
        graph_d3_json["nodes"].append(
            {"name": temp_root_ing_val.name, "count": 12})
        ...
        while(level_of_tree != 0):
            while(len(nodes_to_eval) != 0):
                ...
                overlaps = self.find_children_of_root(
                    temp_root_value, visited_nodes_set, K=K_)

                # source_idx = temp_node_to_idx[temp_root_value.name]
                source_name = temp_root_value.name
                for ing_value, overlap_score in overlaps:

                    # TODO: overlaps doesn't return what we want! we need an
                    # ingredient object
                    graph_d3_json["nodes"].append(
                        {"name": ing_value.name, "size": 12})
    ...
    def find_children_of_root(self, temp_root_ing_val, visited_set, INTERSECT_THRESH=0.4, K=5):
        ...
        for ing_name, ing_value in self.graph.items():
            if(ing_name != "" and ing_name is not None and ing_name != temp_root_ing_val.name and ing_name not in visited_set):
                ...
                intersect_percent = self.threshold_intersect_recipe_sets(
                    temp_root_ing_val.recipes, ing_value.recipes)
                if intersect_percent > INTERSECT_THRESH:
                    overlaps[ing_name] = (ing_value, intersect_percent)

        # [1] gets the value, [0] gets the element in the array with 1 value, [1] gets the intersect_percent
        x = sorted(overlaps.items(), key=lambda x: x[1][1])
    ...
    def get_ingredient_common_score(self, ing1, ing2):
        intersect_percent = self.percent_intersect_recipe_sets(
            (ing1.recipes, ing2.recipes))
    ...
    def harmonic_intersect_recipe_sets(self, root_recipe_set, ing_recipe_set):
        ...
        len_intersect = len(self.intersect_recipe_sets(
            [root_recipe_set, ing_recipe_set]))

        intersect_over_root = (1.0 * len_intersect) / \
            (1.0 * len_root_recipe_set)
        intersect_over_ing = (1.0 * len_intersect) / (1.0 * len_ing_recipe_set)

        if (intersect_over_root + intersect_over_ing) == 0:
            return 0.0
        harmonic_percent_overlap = (
            (2.0 * intersect_over_root * intersect_over_ing) / (intersect_over_root + intersect_over_ing))
    ...
    def percent_intersect_recipe_sets(self, root_recipe_set, ing_recipe_set):
        len_smallest_recipe_set = min(
            len(root_recipe_set), len(ing_recipe_set))
        len_intersect = len(self.intersect_recipe_sets(
            [root_recipe_set, ing_recipe_set]))
