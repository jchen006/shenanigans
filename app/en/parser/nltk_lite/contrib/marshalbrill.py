...
class Brill(TagI):
    ...
    # marshal() and unmarshal() methods by Tiago Tresoldi
    # <tresoldi@users.sf.net>
    ...
    def unmarshal(self, filename):
        ...
        """
        rule_a = re.compile(
            r"^(.+) -> (.+) if the (.+) of words i([+-]\d+)...i([+-]\d+) is '(.+)'$", re.UNICODE)
        rule_b = re.compile(
...
class BrillTrainer(object):
    ...
    def train(self, train_tokens, max_rules=200, min_score=2):
        ...
        test_tokens = list(self._initial_tagger.tag(
            t[0] for t in train_tokens))
...
class FastBrillTrainer(object):
    ...
    def train(self, train_tokens, max_rules=200, min_score=2):
        ...
        tagged_tokens = list(self._initial_tagger.tag(
            t[0] for t in train_tokens))
        ...
        while len(rules) < max_rules and maxScore >= min_score:
            ...
            for rule in bestRules:
                ...
                if ruleScores[rule] == maxScore:
                    ...
                    firstUnknownIndex[rule] = len(tagged_tokens)
