...
from en.parser.nltk_lite.contrib.marshal import MarshalDefault
Default = MarshalDefault
from en.parser.nltk_lite.contrib.marshal import MarshalUnigram
Unigram = MarshalUnigram
from en.parser.nltk_lite.contrib.marshal import MarshalAffix
Affix = MarshalAffix
from en.parser.nltk_lite.contrib.marshal import MarshalNgram
Ngram = MarshalNgram
...
class CombinedTagger (SequentialBackoff):
    ...
    def _append_ngram(self, size, train_sents, verbose=False, cutoff_value=0.001):
        ...
        self._tagger.append(
            Ngram(size, cutoff=cutoff, backoff=self._tagger[-1]))
        self._tagger[-1].train([train_sents], verbose)
    ...
    def marshal(self, basepath):
        # create the model files, one for each tagger (*.mod) plus a general
        # one
