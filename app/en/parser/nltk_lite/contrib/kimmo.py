...
class KimmoGUI:
    ...
    def __init__(self, grammar, text, title='Kimmo Interface v1.78'):
        ...
        try:
            ...
            # change to a window and field eventually.
            self.debugWin = Tkinter.StringVar()
            ...
            Tkinter.Label(level2, text="Generate or Recognize:").pack(
                side='left')
            ...
            scroll = Tkinter.Scrollbar(
                lexiconFrame, orient='horizontal', command=self.lexicon.xview)
            ...
            # setup the scrollbar
            scroll = Tkinter.Scrollbar(
                rulesFrame, orient='horizontal', command=self.rules.xview)
            scroll.pack(side='bottom', fill='x')
            ...
            Tkinter.Button(midbetweenFrame, text='clear',
                           ...
                           command=lambda start=1.0, end=Tkinter.END: self.results.delete(
                               start, end)
                           ).pack(side='right')
            ...
            # setup the scrollbar
            scroll = Tkinter.Scrollbar(
                midFrame, orient='horizontal', command=self.results.xview)
            scroll.pack(side='bottom', fill='x')
            ...
            self.loadMenuButton = Tkinter.Menubutton(
                ctlbuttons, text='Load', background='#a0c0c0', foreground='black', relief='raised')
            self.loadMenuButton.pack(side='left')
            self.loadMenu = Tkinter.Menu(self.loadMenuButton, tearoff=0)

            self.loadMenu.add_command(label='Load Lexicon', underline=0, command=lambda filetype='.lex',
                                      targetWindow=self.lexicon, tf='l': self.loadTypetoTarget(filetype, targetWindow, tf))
            self.loadMenu.add_command(label='Load Rules', underline=0, command=lambda filetype='.rul',
                                      targetWindow=self.rules, tf='r': self.loadTypetoTarget(filetype, targetWindow, tf))
...
#
            ...
            self.saveMenuButton = Tkinter.Menubutton(
                ctlbuttons, text='Save', background='#a0c0c0', foreground='black', relief='raised')
            self.saveMenuButton.pack(side='left')
            self.saveMenu = Tkinter.Menu(self.saveMenuButton, tearoff=0)
            self.saveMenu.add_command(label='Save Lexicon', underline=0, command=lambda filename=self.lexfilename,
                                      sourceWindow=self.lexicon: self.writeToFilefromWindow(filename, sourceWindow, 'w', 0, 'l'))
            self.saveMenu.add_command(label='Save Rules', underline=0, command=lambda filename=self.rulfilename,
                                      sourceWindow=self.rules: self.writeToFilefromWindow(filename, sourceWindow, 'w', 0, 'r'))
            self.saveMenu.add_command(label='Save Results', underline=0, command=lambda filename='.results',
                                      sourceWindow=self.results: self.writeToFilefromWindow(filename, sourceWindow, 'w', 0))
            self.saveMenu.add_command(
                label='Save All', underline=0, command=self.saveAll)
            ...
            self.configsMenuButton = Tkinter.Menubutton(
                ctlbuttons, text='Configs', background='#a0c0c0', foreground='black', relief='raised')
            ...
            for x in currentconfigfiles:
                ...
                self.configsMenu.add_command(label=newname, underline=0, command=lambda newname=x: self.configLoader(
                    newname))  # Callback(self.configLoader,newname))
...
#                           command=self.generate).pack(side='right')
            ...
            self.graphMenuButton = Tkinter.Menubutton(
                ctlbuttons, text='Graph', background='#d0d0e8', foreground='black', relief='raised')
            self.graphMenuButton.pack(side='right')
            self.graphMenu = Tkinter.Menu(self.graphMenuButton, tearoff=0)

            self.graphMenu.add_command(
                label='Graph Lexicon', underline=0, command=lambda which='l': self.graph(which))
            self.graphMenu.add_command(
                label='Graph FSA Rules', underline=0, command=lambda which='r': self.graph(which))
            # self.loadMenu.add_command(label='Load Lexicon', underline=0,command = lambda filetype='.lex', targetWindow = self.lexicon : loadTypetoTarget(self, filetype, targetWindow))
        ...
        if (self.dbgTracing):
            ...
            try:
                ...
                Tkinter.Button(dbgTraceFrame2, text='clear',
                               background='#a0c0c0', foreground='black',
                               command=lambda start=1.0, end=Tkinter.END: self.traceWindow.delete(
                                   start, end)
                               ).pack(side='right')
                Tkinter.Button(dbgTraceFrame2, text='Save',
                               background='#a0c0c0', foreground='black',
                               command=lambda file=self.kimmoResultFile, windowName=self.traceWindow, mode='w', auto=0: self.writeToFilefromWindow(
                                   file, windowName, mode, auto)
                               ).pack(side='left')

                scroll = Tkinter.Scrollbar(
                    dbgTraceFrame, orient='horizontal', command=self.traceWindow.xview)
                scroll.pack(side='bottom', fill='x')
                ...
                self.dbgTracing.protocol(
                    "WM_DELETE_WINDOW", self.create_destroyDebugTracing)
    ...
    def writeToFilefromWindow(self, filename, windowName, mode, auto, wt=None):
        ...
        if filename:
            if wt == 'l':
                self.lexfilename = filename
            elif wt == 'r':
                self.rulfilename = filename
    ...
    def configLoader(self, *args):
        ...
        if filename:
            ...
            for line in lines:
                line = line.strip()
                cfgargs = line.split(":")
                for x in range(len(cfgargs)):
                    cfgargs[x] = cfgargs[x].strip()
                ...
                if len(line) == 0:
                    ...
                    self.lexfilename = self.loadIntoWindow(
                        os.path.expanduser(cfgargs[1]), self.lexicon)
                elif cfgargs[0] == 'rules':
                    self.rulfilename = self.loadIntoWindow(
                        os.path.expanduser(cfgargs[1]), self.rules)
                ...
                elif cfgargs[0] == 'batch':
                    self.batchpath.set(os.path.expanduser(cfgargs[1]))
                # !
                else:
                    self.guiError('unknown line :' + line)
                # print line

        else:
            self.guiError('Empty Filename')
    ...
    def loadIntoWindow(self, filename, windowField):
        ...
        # opens a load dialog for files of a specified type to be loaded into a
        # specified window
    def loadTypetoTarget(self, fileType, targetWindow, ftype=None):

        if not (fileType and targetWindow):
            return
...
# need to make the kimmo class capable of being interrupted & resumed.
    ...
    def batch(self, *args):
        ...
        if filename:
            ...
            for line in lines:
                                                    # a 'g word' 'r word'
                                                    # format
                # should be a single word, no spaces, etc.
                singleword = line.strip()
                spcr = re.compile(r"\s+")
                linevals = []
                ...
                if len(batch_result) > 0:
                    for x in batch_result:
                        batch_result_str = batch_result_str + x
            ...
            self.results.insert(
                1.0, '----- ' + time.strftime("%a, %d %b %Y %I:%M %p", time.gmtime()) + ' -----\n')
            self.results.insert(2.0, results_string)
            self.results.see(1.0)

            if self.traceWindow:
                self.highlightMatches(
                    '    BLOCKED', self.traceWindow, '#ffe0e0')
                self.highlightMatches(
                    '      AT END OF WORD', self.traceWindow, '#e0ffe0')
    ...
    def generate(self, *args):
        ...
        if len(self.wordIn.get()) > 0:
            ...
            for x in generate_result:
                generate_result_str = generate_result_str + x
    ...
    def recognize(self, *args):
        ...
        if len(self.wordIn.get()) > 0:
            ...
            # convert list to string
            for x in recognize_result:
                recognize_result_str = recognize_result_str + x
    ...
    def graph(self, which):
        ...
        if not ((pathstatus[0] & 0600) == 0600):
            path = '/tmp/' + str(os.environ.get("USER")) + \
                '/'  # need terminating /
        ...
        if which == 'l':
            ...
            dotstring = dotformat(self.kimmoinstance.fsasNodes[
                                  string.atoi(ld.result[0])]['nodes'])
            graphtitle = 'FSA ' + \
                self.kimmoinstance.fsasNodes[string.atoi(ld.result[0])]['name']

            # make file read:
            # something.rul.1.gif  (where 1 is the rule index number)
            graphfname += ('.' + str(ld.result[0]))

            # check if that file already exists, if so, append an iteration
            # number onto it.
        ...
        if leximagefile:
            if self.lexicalGraphWindow:
                self.lexicalGraphWindow.destroy()
    ...
    def validate(self, *args):
        ...
        for x in self.lexicon.tag_names():
            self.lexicon.tag_delete(x)
    ...
    def highlightMatches(self, word, window, color):
        ...
        matchRight = '1.0'
        while matchIdx != '':
            matchIdx = window.search(
                word, matchRight, count=1, stopindex=Tkinter.END)
            if matchIdx == '':
                break

            strptr = matchIdx.split(".")
            matchRight = strptr[0] + '.' + \
                str((int(strptr[1], 10) + len(word)))

            window.tag_add(self.tagId, matchIdx, matchRight)
            window.tag_configure(
                self.tagId, background=color, foreground='black')
...
# INIT KIMMO
    def initKimmo(self, *args):
        ...
        if (not self.kimmoinstance) or (self.rulemd5 != tmprmd5) or (self.lexmd5 != tmplmd5):
            ...
            self.kimmoinstance = KimmoControl(self.lexicon.get(
                1.0, Tkinter.END), self.rules.get(1.0, Tkinter.END), '', '', self.debug)
            self.guiError("")
...
# redirect to debug window, if operational
    def capturePrint(self, *args):
        ...
        # no no, if tracing is on, but no window, turn tracing off and cleanup
        # window

        # !!! if tracing is on, but window is not defined, create it.
            # this will cause a post-recover from an improper close of the
            # debug window
    ...
    def kimmoHelp(self, *args):
        ...
        # setup the scrollbar
        scroll = Tkinter.Scrollbar(
            helpWindow, orient='horizontal', command=help.xview)
        scroll.pack(side='bottom', fill='x')
...
class PrintHook:
                    ...
                    if self.out:
                        if lineNo:
                            try:
                                ...
                                newText = 'line(' + str(sys.exc_info()
                                                        [2].tb_frame.f_back.f_lineno) + '):' + newText
    ...
    # pass all other methods to __stdout__ so that we don't have to override
    # them
...
class ListDialog(Tkinter.Toplevel):
    ...
    def __init__(self, parent, listOptions, title=None):
        ...
        Tkinter.Label(box, text="Select an FSA to graph").pack(
            side='top', fill='x')
        box.pack()
    ...
    def listbox(self, listOptions):
        box = Tkinter.Frame(self)
        self.lb = Tkinter.Listbox(box, height=len(
            listOptions), width=30, background='#f0f0ff', selectbackground='#c0e0ff', selectmode='single')
        self.lb.pack()
...
def dotformat(nodeEdgeAry):
    ...
    graphWH = '4,4'
    if len(nodeEdgeAry) > 3:
        graphWH = '5,5'
    if len(nodeEdgeAry) > 5:
        graphWH = '6,6'
    if len(nodeEdgeAry) > 7:
        graphWH = '7,7'
    if len(nodeEdgeAry) > 10:
        graphWH = '7.5,7.5'
    ...
    for x in nodeEdgeAry:
        if x['node'] == 'Begin':
            features = ' [' + 'shape=box,color=lightblue,style=filled] '
        elif x['node'] == 'End':
            features = ' [' + 'color="Light Coral",style=filled] '
        elif x['features']:
            features = ' [' + x['features'] + '] '
        elif not x['features']:
            features = ''
        ...
        for e in range(len(x['edges'])):
            ...
            if e < len(x['edgenames']):
                dotstring += ('[label="\l' + x['edgenames'][e] + '"]')
            dotstring += ";\n"
...
def dot2image(filename, dotstring):
    ...
    os.system('pstopnm -stdout -portrait -ppm ' +
              psfilename + ' > ' + imgfilename)
...
class KimmoControl:
    ...
    def __init__(self, lexicon_string, rule_string, lexicon_file, rule_file, debug):
        ...
        # a better way is just to use a destructor and check if the object
        # exists.
        self.ok = 0
        self.errors = ''
    ...
    def generate(self, word):
        if self.boundary_char:
            word += self.boundary_char
        genlist = _generate_test(self.s, word)

        genliststr = genlist.__repr__()
        if self.boundary_char:
            genliststr = genliststr.replace(self.boundary_char, '')
    ...
    def processLexicon(self, text):
        ...
        for item in text.split("\n"):
            ...
            # LEXICON N_ROOT1
                ...
                if (len(lexigroup) > 0):
                    if len(kimmoWords):
                        # print lexigroup
                        # print kimmoWord
                        self.klexicons.append(
                            KimmoLexicon(lexigroup, kimmoWords))
                        self.lexicalNodes.append(tmpnode)
                    kimmoWords = []
                # remove trailing ':'  , new group
                lexigroup = cleanLine[0:len(cleanLine) - 1]
            ...
            # assume that a : contained in the line that is not a last char
            # means it is an alternation.
            elif ':' in cleanLine:
                ...
                p = re.compile(r"\s+")
                moreitems = []
                # moreitems = item.split(" ")                     # make sure
                # to add tabs and other whitespace..
                ...
                for x in range(len(moreitems)):
                    # print moreitems[x]
                    if (moreitems[x][0] == '"') and (rangestart < 0):
                        rangestart = x
                    elif (moreitems[x][len(moreitems[x]) - 1] == '"') and (rangestart > -1):
                        rangeend = x
                        moreitems[rangestart] = string.join(
                            moreitems[rangestart:rangeend + 1], " ")
                ...
                for furtheritem in moreitems:
                    ...
                    if not len(moreitems[i]):
                        continue
                    if i > 2:
                        continue
                    else:
                        testlex.append(moreitems[i])
                    i += 1
                ...
                if len(moreitems) > 2:
                    kimmoWords.append(
                        KimmoWord(moreitems[0], moreitems[2], moreitems[1]))
                    self.validateLexicon.append(moreitems[1])
                    # print moreitems
                elif len(moreitems) > 1:
                    kimmoWords.append(
                        KimmoWord(moreitems[0], '', moreitems[1]))
                    self.validateLexicon.append(moreitems[1])
    ...
    def processAlternations(self, text):
        ...
        for line in text.split("\n"):
            ...
            if len(cleanLine) == 0:
                ...
                # lets break it first, then keep on looping until we find the
                # next group (signified by a : )
                p = re.compile(r"\s+")
                items = []
                ...
                for item in items:
                    ...
                    # ALTERNATION V_root
                        ...
                        # all all prior alternations to prior altgroup (if
                        # defined)
                        if len(altgroup) > 0:
                            if len(kimmoAlts) > 0:
                                ...
                                for x in kimmoAlts:
                                    self.validateAlternations.append(x)
                                self.lexicalNodes.append(tmpnode)
                        ...
                        tmpnode[
                            'features'] = 'color=\"aquamarine2\", style=filled'
                        tmpnode['edges'] = []
                        tmpnode['edgenames'] = []
    ...
    def processRules(self, text):
        ...
        for line in text.split("\n"):
            ...
            # also boundary would need to be defined before ...
                ...
                # also set up the @ subset if alphabet is defined (old rule
                # file style)
                elif items[0] == 'ALPHABET':
                    anyset[1] = string.join(items[1:len(items)], " ")
                ...
                # using the boundary char, set the final boundary & also add to
                # the any set.
                    ...
                    self.kdefaults = [KimmoDefaults(
                        string.join(items[1:len(items)], " "))]
                    ...
                    # ")
                    self.krules.append(
                        KimmoArrowRule(items[1],  string.join(
                            items[2:len(items)], " "))
                    ...
                    if rulegroup:
                        self.guiError('error, fsa rule not finished')
                    ...
                    tmpnode['node'] = '0'
                    tmpnode[
                        'features'] = 'color="indianred1", style=filled, shape=box'
                    tmpnode['edges'] = []
                    tmpnode['edgenames'] = []
                    ...
                    # get row1 and row2 of text & translate into x:y col
                    # format.
                    ...
                    if ((':' in items[0]) or ('.' in items[0])) and (not ct.match(items[0])):
                        ...
                        # sprint items[0][0:len(items[0])-1] + ' -- ' +
                        # string.join(items[1:len(items)], " ")

                        if (items[0][len(items[0]) - 1] == ':'):
                            finalstate = True
                        elif (items[0][len(items[0]) - 1] == '.'):
                            finalstate = False
                        else:
                            self.guiError(
                                "FSA table failure -- 'final state defn'")
                            continue

                        items[0] = items[0][
                            0:len(items[0]) - 1]  # remove the ':'

                        # convert to integers (instead of strings)
                        for x in range(rulecolcnt + 1):
                            # including the first row number - i.e. '4:'
                            items[x] = string.atoi(items[x])

                        # add this row.
                        kimmoRule.append(
                            (items[0], finalstate, items[1:len(items)]))

                        # now make this row into graph transitions
                        tmpnode = {}		# a node and its edges
                        tmpnode['node'] = str(items[0])
                        tmpnode[
                            'features'] = 'shape=box, fillcolor="lavender blush", style=filled'
                        if finalstate and (items[0] == 1):
                            tmpnode[
                                'features'] = 'shape=circle, color="paleturquoise2", style=filled'
                        elif (items[0] == 1):
                            tmpnode[
                                'features'] = 'color="paleturquoise2", style=filled, shape=box'
                        elif (finalstate):
                            tmpnode[
                                'features'] = 'shape=circle,fillcolor="honeydew2", style=filled'
                        tmpnode['edges'] = []
                        tmpnode['edgenames'] = []
                        # add as strings
                        ...
                        # if number is equal to row count, then add total and
                        # reset rule group
                    ...
                    # the char class/translations
                        ...
                        if (len(ruleTo) != rulecolcnt) or (len(ruleFrom) != rulecolcnt):
                            continue
                        else:
                            for x in range(rulecolcnt):
                                if ruleTo[x] == ruleFrom[x]:
                                    ruleTran.append(ruleTo[x])
                                else:
                                    ruleTran.append(
                                        ruleFrom[x] + ':' + ruleTo[x])
        ...
        if (anyset[0] and anyset[1]):
            self.ksubsets.append(KimmoSubset(
                anyset[0], string.join(anyset[1:len(anyset)], " ")))
...
class KimmoPair:
    ...
    def matches(self, input, output, subsets, negatedOutputMatch=False):
        if not(self._matches(self.input(), input, subsets)):
            return False
        m = self._matches(self.output(), output, subsets)
        if negatedOutputMatch:
            return not(m)
        return m
...
# \}\{\<\>\,\.\~ # (^|\s)?\*(\s|$) !!! * is already covered in the re tokenizer
_kimmo_terminal_regexp = '[a-zA-Z0-9\+\'\-\#\@\$\%\!\^\`\}\{]+'
# for FSA, only invalid chars are whitespace and :
...
_kimmo_rule = _kimmo_terminal_regexp_ext + \
    '|[\:\(\)\[\]\?\&\*\_]|<=>|==>|<==|/<='
...
def parse_pair_sequence(description, token_type):
    ...
    for token in desc:
        if token == ':':
            if colon:
                raise ValueError('two colons in a row')
            if prev == None:
                raise ValueError('colon must follow identifier')
            colon = True
        elif colon:
            ...
            colon = False
    if colon:
        raise ValueError('colon with no following identifier')
    if prev:
        result.append(KimmoPair(prev, prev))
    return result
...
class KimmoSubset:
    ...
    def __init__(self, name, description):
        ...
        self._subset = list(
            set(tokenize.regexp(description, _kimmo_terminal_regexp_fsa)))
...
class KimmoArrowRule:
            ...
            for next_state in fsa.forward_traverse(current_state):
                ...
                for pair in fsa._labels[(current_state, next_state)]:
                    if pair.matches(input, output, subsets):
                        ...
                        break
                if (ok):
                    if (next_state in fsa.finals()):
                        contains_halt_state = True
                    if not(next_state in result):
    ...
    def _parse_pair(self, tokens, i):
        ...
        if t1 in _special_tokens:
            raise ValueError('expected identifier, not ' + t1)
        t2 = t1
        j = i + 1
        if self._next_token(tokens, j) == ':':
            t2 = self._next_token(tokens, j + 1, True)
            if t2 in _special_tokens:
                raise ValueError('expected identifier, not ' + t2)
            j = j + 2
            tree = Tree('Pair', tokens[i:j])
        else:
            tree = Tree('Pair', [tokens[i]])
        # print str(self._pair_from_tree(tree)) + ' from ' + str(i) + ' to ' +
        # str(j)
        return (j, tree)
    ...
    def _collect_alphabet(self, tree, sigma):
        if tree.node == 'Pair':
            ...
            for d in tree:
                self._collect_alphabet(d, sigma)
    ...
    def _parse_singleton(self, tokens, i):
        ...
        if t == '(':
            ...
            if result == None:
                raise ValueError('missing contents of (...)')
            t = self._next_token(tokens, j, True)
            if t != ')':
                raise ValueError(
                    'missing final parenthesis, instead found ' + t)
            j = j + 1
        elif t == '[':
    ...
    def _build_terminal(self, fsa, entry_node, terminal):
        ...
        # print '_build_terminal(%d,%s) -> %d' % (entry_node, terminal,
        # new_exit_node)
    ...
    def _build_seq(self, fsa, node, tree0, tree1, reverse):
        ...
        # print '_build_seq(%d,%s,%s) -> %d,%d' % (node, tree0, tree1, node1,
        # node2)
        return node2
...
class KimmoFSARule:
    ...
    def __init__(self, name, pair_description, state_descriptions):
        ...
        for (index, is_final, next_state_array) in state_descriptions:
            if not(is_final == True or is_final == False):
                raise ValueError(
                    'each state description must take the form (index, True/False, [next_state_indices...]')

            if len(next_state_array) != len(self.pairs()):
                raise ValueError('transition array of wrong size ' +
                                 str(len(next_state_array)) + ' ' + str(len(self.pairs())))
    ...
    def sorted_pairs(self, subsets):
        ...
        sorted_with_index = []
        # enumerate lists all & assigns an index
        ...
        sorted_with_index.sort(lambda x, y: self.mycompare(
            x[2], y[2], x[3], y[3]))  # lambda x, y: x[2] - y[2])
    ...
    def right_advance(self, current_states, input, output, subsets):
        ...
        for index in current_states:
            ...
            # enumerate(self.pairs()):
            for i, pair, size1, size2 in self.sorted_pairs(subsets):
                ...
                if pair.matches(input, output, subsets):
                    ...
                    if 0:  # ('@' in pair.__repr__()):
                        ...
                        if self.transitions.has_key(self.transitions[index][i]):
                            ft = self.is_final[self.transitions[index][i]]
                        else:
                            ft = ''
                        any_next_states_ary.append(
                            [i, self.transitions[index][i], ft, pair.__repr__()])
...
class KimmoLexicon:
    ...
    # tree is ([KimmoWord], [ (char, sub-trie), ... ])
    def trie(self): return self._trie
    ...
    def build_trie(self, words, word_position=0):
        ...
        first_chars = {}
        for w in words:
            if len(w.letters()) <= word_position:
                continue
        ...
        for c, sub_words in first_chars.items():
            sub_tries.append(
                (c, self.build_trie(sub_words, word_position + 1)))
...
class KimmoMorphology:
    ...
    def __init__(self, alternations, lexicons, start='Begin'):
        ...
        for a in alternations:
            self.alternations[a.name()] = a
        for l in lexicons:
            self.lexicons[l.name()] = l
    ...
    def _possible_next_characters(self, state, chars):
        for s in state:
            if isinstance(s, KimmoLexicon):
                ...
                (words, sub_tries) = s
            for w in words:
                self._possible_next_characters(
                    self._collect(w.next_alternation()), chars)
...
class KimmoRuleSet:
        ...
        for kd in defaults:
            for pair in kd.defaults():
                ...
                if self.is_subset(pair.input()) or self.is_subset(pair.output()):
                    raise ValueError(
                        'default ' + str(pair) + ' contains subset')
    ...
    def _evaluate_rule_left_context(self, rule, input, output):
        ...
        while i >= 0:
            next_states = []
            (result, contains_halt_state) = rule.advance(
                fsa, states, input[i], output[i], self.subsets())
            if contains_halt_state:
                return True
            for s in result:
                if not(s in next_states):
    ...
    def _debug_print_input_and_output(self, position, rule_states, morphological_state,
        ...
        if (self.debug):
            ...
            # print '    {%d %s %s %s}' % (start, rule, fsa_states,
    ...
    def _generate(self, input_tokens, position, rule_states, morphological_state, input, output, result_str, result_words,
        ...
        if ((position >= len(input_tokens))):  # and (not morphological_state)
            ...
            if (self.debug):
               ...
               # also, recognizer is failing to put on the added information
               # like "+genetive"
            ...
            if morphological_state:
                    ...
                    # print 'morpho'
                possible_next_input_chars = invert.possible_next_characters(
                    morphological_state)
                ...
                if ('0' in possible_next_input_chars) or ('#' in possible_next_input_chars):
                    if '0' in possible_next_input_chars:
                        boundary = '0'
                    elif '#' in possible_next_input_chars:
                        boundary = '#'
                ...
                # GENERATION CASE
                # print 'no-morpho'
                self._debug_print_input_and_output(
                    position, rule_states, morphological_state, input, output, None, None, invert)
                for (start, rule, fsa_states, required_truth_value) in rule_states:
            ...
            for pair_input, pair_output in self._pair_alphabet:
                ...
                if invert:
                        # check if the output of a transition is in the input
                        # string (input_tokens)
                    compare_token = pair_output
                else:
                    compare_token = pair_input

                if not(compare_token == self.null() or compare_token == input_tokens[position]):
                    continue
                ...
                for s in rule_states:
                    ...
                    current_state_str = '['
                    for x in fsa_state_set:
                        current_state_str += str(x)
                    ...
                    current_state_str = ''
                    for x in next_fsa_state_set:
                        current_state_str += str(x)
                    if not current_state_str:
                        current_state_str = '0 (FAIL)'
                    rule_state_debug += ('->' + current_state_str + '] ')
                    ...
                    if (contains_halt_state == True and isinstance(rule, KimmoArrowRule)):
                        ...
                        # truth is evaluated by following the states until the
                        # end.
                        ...
                        next_rule_states.append(
                            (start, rule, next_fsa_state_set, required_truth_value))

                if (self.debug):
                    print rule_state_debug
                ...
                for rule in self.rules():
                    if not(isinstance(rule, KimmoArrowRule)):
                        continue

                    required_truth_value = rule.matches(
                        pair_input, pair_output, self.subsets())
                    if required_truth_value == None:
                        continue
                    left_value = self._evaluate_rule_left_context(
                        rule, input, output)
                    if (left_value == False):
                        if (required_truth_value == True):
                            fail = rule
                    ...
                    if (rule.rightFSA()):
                        ...
                        next_rule_states.append(
                            (position, rule, [rule.rightFSA().start()], required_truth_value))
                    else:
                        if (required_truth_value == False):
                            fail = rule
                            continue
                ...
                if (fail == None):
                    ...
                    # as it is, it only yields its morph if there is a valid
                    # next morphology
...
if __name__ == '__main__':
    ...
    for x in sys.argv:
        ...
        if ".lex" in x:
            ...
            filename_batch_test = x
        elif x[0:3] == "-r:":
            recognize_string = x[3:len(x)]
        elif x[0:3] == "-g:":
            generate_string = x[3:len(x)]
        elif x == "debug":
            console_debug = 1
    ...
    if (recognize_string or generate_string or filename_batch_test) and filename_rul:
        kimmoinstance = KimmoControl(
            "", "", filename_lex, filename_rul, console_debug)
