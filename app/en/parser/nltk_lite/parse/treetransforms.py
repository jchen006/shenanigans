        ...
        if isinstance(node, Tree):
            ...
            if len(node) > 2:
                ...
                for i in range(1, numChildren - 1):
                    if factor == "right":
                        newHead = "%s%s<%s>%s" % (originalNode, childChar, "-".join(
                            childNodes[i:min([i + horzMarkov, numChildren])]), parentString)  # create new head
                        newNode = Tree(newHead, [])
                        curNode[0:] = [nodeCopy.pop(0), newNode]
                    else:
                        newHead = "%s%s<%s>%s" % (originalNode, childChar, "-".join(
                            childNodes[max([numChildren - i - horzMarkov, 0]):-i]), parentString)
                        newNode = Tree(newHead, [])
        ...
        if isinstance(node, Tree):
            ...
            if childIndex != -1:
                ...
                if expandUnary == True:
                    ...
                    if unaryIndex != -1:
                        newNode = Tree(
                            node.node[unaryIndex + 1:], [i for i in node])
