...
class Category(FeatureStructure, cfg.Nonterminal):
    ...
    def __setitem__(self, name, value):
        if self._frozen:
            raise "Cannot modify a frozen Category"
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        for fname in items:
            if fname == self.__class__.headname:
                continue
            fval = self[fname]
    ...
    def _str(self, reentrances, reentrance_ids):
        ...
        for fname in items:
            ...
            if not isinstance(fval, FeatureStructure):
                ...
                fval_lines = [(' ' * (maxfnamelen + 3)) +
                              l for l in fval_lines]
        ...
        for l in range(len(lines)):
            ...
            if l == headline:
                lines[l] = ('%s[ %s%s ]' %
                            (head, line, ' ' * (maxlen - len(line))))
            else:
                lines[l] = ('%s[ %s%s ]' %
                            (' ' * len(head), line, ' ' * (maxlen - len(line))))
    ...
    def _parse(cls, s, position=0, reentrances=None):
        ...
        if match is not None:
            if head is None:
                ...
                return cls(**{cls.headname: head}), match.end()
    ...
    def _parseval(cls, s, position, reentrances):
        ...
        # Semantic value of the form <app(?x, ?y) >'; return an
        ...
        # other semantic value enclosed by '< >'; return value given by the
        # lambda expr parser
    ...
    def parse_rules(cls, s):
        ...
        if match is None:
            raise ValueError('arrow', position)
        else:
            position = match.end()
        rules = []
        while position < len(s):
...
class GrammarCategory(Category):
    ...
    def _repr(self, reentrances, reentrance_ids):
        ...
        # never fall through to comparing values.
        for fname in items:
            if fname == self.__class__.headname or fname == '/':
                continue
            fval = self[fname]
            if isinstance(fval, bool):
                if fval:
    ...
    _PARSE_RE['categorystart'] = re.compile(
        r'\s*([^\s\(\)"\'\-=,\[\]/]*)\s*(\[|/)')
