...
class IndVariableExpression(VariableExpression):
    ...
    def __init__(self, variable):
        ...
        assert is_indvar(
            str(variable)), "Wrong format for an Individual Variable: %s" % variable
        self.variable = variable
...
class ApplicationExpression(Expression):
    ...
    def __repr__(self): return "ApplicationExpression('%s', '%s')" % (
...
def runtests():
    ...
    assert v4 == four
    v5 = ApplicationExpression(
        pred, ApplicationExpression(pred, v4)).simplify()
    assert v5 == two
    ...
    x1 = Parser(
        r'some x.(and (man x) (\p.some x.(and (woman x) (p x)) \y.(love x y)))').next().simplify()
    x2 = Parser(
        r'some x.(and (man x) some y.(and (woman y) (love x y)))').next().simplify()
    assert x1 == x2
    ...
    x2 = Parser(
        r'(and (probably (walk vincent)) (improbably (talk mia)))').next().simplify()
    assert x1 == x2

    x1 = Parser(
        r'(\a.\b.(\c.(c a b) \d.\e.(love d e)) jules mia)').next().simplify()
    x2 = Parser(r'(love jules mia)').next().simplify()
    assert x1 == x2

    x1 = Parser(
        r'(\a.\b.some c.(and (a c) (b c)) \d.(boxer d) \d.(sleep d))').next().simplify()
    ...
    x1 = Parser(
        r'(\a.\b.(\c.(c a b) \b.\a.(loves b a)) jules mia)').next().simplify()
    x2 = Parser(r'(loves jules mia)').next().simplify()
    assert x1 == x2

    x1 = Parser(
        r'(\a.\b.(and some b.(a b) (a b)) \c.(boxer c) vincent)').next().simplify()
